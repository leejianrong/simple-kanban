...
def test_get_card_by_id(client):
    ...
    r = client.get(f"/api/v1/cards/{created['id']}")
    assert r.status_code == 200
    assert r.json() == created


def test_get_missing_card_404(client):
    assert client.get("/api/v1/cards/999").status_code == 404
...
def test_patch_is_partial(client):
    ...
    r = client.patch(f"/api/v1/cards/{card['id']}", json={"title": "New title"})
    body = r.json()
...
def test_patch_rejects_empty_or_null_title(client):
    card = _create(client)
    assert client.patch(f"/api/v1/cards/{card['id']}", json={"title": ""}).status_code == 422
    assert client.patch(f"/api/v1/cards/{card['id']}", json={"title": "   "}).status_code == 422
    assert client.patch(f"/api/v1/cards/{card['id']}", json={"title": None}).status_code == 422


def test_patch_rejects_bad_story_points(client):
    card = _create(client)
    assert client.patch(f"/api/v1/cards/{card['id']}", json={"story_points": 4}).status_code == 422
...
def test_patch_missing_card_404(client):
    assert client.patch("/api/v1/cards/999", json={"title": "x"}).status_code == 404
...
def test_delete_missing_card_404(client):
    assert client.delete("/api/v1/cards/999").status_code == 404
