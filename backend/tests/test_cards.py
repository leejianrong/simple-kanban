"""API tests for the endpoints that exist today: GET /api/cards, POST /api/cards.
...
def test_create_card_minimal_defaults(client):
    r = client.post("/api/cards", json={"title": "First card"})
...
def test_ticket_numbers_increment(client):
    ...
    c = client.post("/api/cards", json={"title": "C"}).json()
...
def test_append_position_is_per_column(client):
    t0 = client.post("/api/cards", json={"title": "t0", "column": "todo"}).json()
    t1 = client.post("/api/cards", json={"title": "t1", "column": "todo"}).json()
    d0 = client.post("/api/cards", json={"title": "d0", "column": "done"}).json()
...
def test_list_returns_all_created(client):
    client.post("/api/cards", json={"title": "A"})
    client.post("/api/cards", json={"title": "B", "column": "done"})
...
def test_validation_rejects_unknown_column(client):
    assert client.post("/api/cards", json={"title": "x", "column": "backlog"}).status_code == 422
